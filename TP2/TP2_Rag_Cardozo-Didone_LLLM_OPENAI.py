...
with st.sidebar:
    ...
    openai_key    = st.text_input("OpenAI API Key",     type="password", placeholder="sk-...")  # ← OpenAI
    ...
    st.markdown(
        ...
        "RAG · LangChain · Pinecone · OpenAI</span>",
        unsafe_allow_html=True,
    )
...
def build_chain(openai_key, pinecone_key, index_name, namespace):
...
def ingest_documents(files, openai_key, pinecone_key, index_name, namespace):
    all_docs = []
    names = []
    for f in files:
    ...
    pc.create_index(index_name, dimension=1536, metric="cosine", spec=spec)  # ← 1536 para OpenAI
    time.sleep(5)
        ...
        with st.spinner("Procesando documentos e indexando en Pinecone…"):
            try:
                ...
                names = ingest_documents(
                    uploaded_files, openai_key, pinecone_key, index_name, namespace
                )
                st.session_state.ingested_names = names
                st.session_state.conv_chain     = build_chain(
                    openai_key, pinecone_key, index_name, namespace
                )
...
if not st.session_state.chain_ready:
    if openai_key and pinecone_key:
        try:
            st.session_state.conv_chain  = build_chain(openai_key, pinecone_key, index_name, namespace)
            st.session_state.chain_ready = True
        except Exception:
            pass
